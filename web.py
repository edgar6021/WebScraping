...
with open('jugadores.csv', mode='w', newline='') as file:
    ...
    writer.writerow(['Jugador', 'Temporada', 'Salario'])
    for i in range(len(player_data)):
        for j in range(len(player_data[i][1])):
            if player_data[i][1][j] != '':
                writer.writerow([player_data[i][0], player_data[i][2][j], player_data[i][1][j]])

# Calcular el salario promedio de cada temporada de cada jugador y guardar los datos en el mismo archivo CSV
with open('jugadores.csv', mode='a', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['Jugador', 'Temporada', 'Salario promedio'])
    for i in range(len(player_data[0][1])):
        total_salary = 0
        count = 0
        for j in range(len(player_data)):
            salary = player_data[j][1][i].replace('$', '').replace(',', '')
            if salary != '':
        ...
        if count > 0:
            ...
            average_salary = 0
        writer.writerow([player_data[0][0], player_data[0][2][i], '${:,.2f}'.format(average_salary)])
